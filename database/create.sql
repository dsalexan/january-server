...
SELECT B.student, B.materia, B.status, B.timestamp, CASE WHEN B.position IS NULL THEN NULL ELSE B.position + M.inscritos END AS position, M.weekday, M.starttime, M.endtime
FROM (
  WITH stack AS (
